...
const columns: GridColDef[] = [
  ...
  { field: 'model_item_name', headerName: 'Product', width: 200 },
  {
    ...
    headerName: 'Status',
    width: 200,
    renderCell: (params) => <StatusBadge status={params.value as RepairStatus} size="small" />,
  },
  {
  ...
  },
  {
    ...
    renderCell: (params) => (
      <CurrentLocationBadge
        ...
        centerName={params.row.current_center?.name}
        size="small"
      />
    ),
  },
  {
    field: 'pickup_center',
    headerName: 'Pickup Center',
    width: 140,
    valueGetter: (params: GridValueGetterParams) => params.row.pickup_center?.name || '-',
  },
  {
...
function RepairsContent() {
  ...
  const fetchRepairs = async () => {
    ...
    if (centerFilter) {
      query = query.or(
        `current_center_id.eq.${centerFilter},pickup_center_id.eq.${centerFilter}`
      );
    }

    const sanitizedSearch = sanitizeRepairSearchQuery(debouncedSearchQuery);
    if (sanitizedSearch) {
      const { data: matchingCenters } = await freshClient
        .from('centers')
        .select('id')
        .ilike('name', `%${sanitizedSearch}%`);

      const orClause = buildRepairSearchOrClause(
        sanitizedSearch,
        matchingCenters?.map((center) => center.id) || []
      );
    ...
    if (error) {
      ...
      if (sanitizedSearch) {
        const { data: fallbackData, error: fallbackError } = await freshClient
          .from('repairs')
          .select(`
            *,
            current_center:centers!repairs_current_center_id_fkey(id, name),
        ...
        data =
          fallbackData?.filter((repair) => repairMatchesSearch(repair, sanitizedSearch)) || [];
      } else {
  ...
  useEffect(() => {
    ...
    if (centerParam) {
      setCenterFilter(centerParam);
    }
  }, [statusParam, dateParam, startDateParam, endDateParam, centerParam]);
  ...
  }, [statusFilter, dateFilter, startDateFilter, endDateFilter, searchQuery, centerFilter]);

  // Fetch repairs on mount and when filters change
  useEffect(() => {
    fetchRepairs();
  }, [debouncedSearchQuery, statusFilter, dateFilter, startDateFilter, endDateFilter, showDateRange, centerFilter]);
  ...
  return (
    <Box>
      {fetchError && (
        <Box sx={{ mb: 2 }}>
          <Chip label={`Search error: ${fetchError}`} color="error" variant="outlined" />
        </Box>
      )}
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6} md={3}>
          <TextField
            fullWidth
            placeholder="Search name, phone, repair ID, serial no., email..."
            value={searchQuery}
      ...
      <PageShell
        ...
        actions={
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Button component={Link} href="/dashboard/repairs/new" variant="contained" startIcon={<AddIcon />}>
              New Repair
            </Button>
          </Box>
        }
